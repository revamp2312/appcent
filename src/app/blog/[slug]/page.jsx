      ...
      <div className="w-full max-w-[1440px] flex flex-col justify-center items-center gap-12">
        <div className="flex flex-col gap-4 justify-center items-center">
          <div className="flex gap-1 justify-center items-center">
            <Link href={"/insight"}>
              <h6 className="text-[var(--text-color)]">Customer Stories</h6>
            </Link>
            <RiArrowRightSLine />
            <h6>{blogData?.data?.attributes?.title}</h6>
          </div>
          <div className="flex justify-center items-center text-center"><h2>{blogData?.data?.attributes?.title}</h2></div>
          <div><p>{blogData?.data?.attributes?.DateInText} by Appcentric Solutions, Inc.</p></div>
        </div>
        <div className="max-w-[1248px] w-full  h-full">
          <Image className="w-full max-h-[500px] object-cover"  objectFit="cover" src={ imgurl} alt="Blog-img" width={1248} height={500}/>
        </div>
        <div className="flex flex-col gap-4 max-w-[718px] w-full">
          {description.map((eachPara,index)=>{
            return(
              <p key={index}>{eachPara?.children[0]?.text}</p>
            )
          })}
        </div>
      </div>
